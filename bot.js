/* ============================================================
   Capital Hipotecario — bot.js  ("Capi")
   Asistente sobre créditos hipotecarios. Las preguntas rápidas
   viven DENTRO del área con scroll, para que las respuestas
   siempre queden visibles (también en celular).
   ============================================================ */
(function(){
  function waLink(){
  ...
  const KB = {
    ...
    pie:{keys:["cuanto pie","pie necesito","pie inicial","de pie","el pie","financiamiento","financiar","cuanto financian","cuanto se financia"],
      a:"El <b>pie</b> es la parte del valor que pagas con tus ahorros; el resto lo financia el crédito. Trabajamos con financiamiento de <b>80%, 85% o 90%</b>:<br>• 80% → pie 20%<br>• 85% → pie 15%<br>• 90% → pie 10%<br>Mientras menos pie, mayor el crédito y el dividendo. Considera además los gastos operacionales (2%–4% del crédito). Compara los escenarios en el <a href='simulador.html'>simulador</a>."},
    dividendo:{keys:["dividendo","cuota","cuanto pago","cuanto pagaria","mensual","pago mensual","como se calcula"],
      a:"El <b>dividendo</b> es tu cuota mensual. Depende del valor de la propiedad, cuánto financias, el plazo y la tasa. Calcúlalo al instante en el <a href='simulador.html'>simulador</a>."},
    ...
    cae:{keys:["cae","carga anual","costo total del credito","costo real"],
      a:"El <b>CAE</b> (Carga Anual Equivalente) resume el costo real del crédito incluyendo seguros y gastos, por eso suele ser algo mayor que la tasa. Es la mejor cifra para comparar ofertas entre bancos."},
    ...
    seguros:{keys:["seguro","seguros","desgravamen","incendio","sismo"],
      a:"Todo crédito lleva <b>seguro de desgravamen</b> (cubre la deuda en caso de fallecimiento) y <b>seguro de incendio y sismo</b> (protege la propiedad)."},
    gastos:{keys:["gastos","operacionales","notaria","tasacion","escritura","gop","gastos asociados"],
      a:"Los <b>gastos operacionales</b> (tasación, estudio de títulos, notaría, impuesto al mutuo e inscripción) se pagan una vez al inicio y suman entre <b>2% y 4%</b> del crédito."},
    ...
    refinanciar:{keys:["refinanciar","portabilidad","cambiar de banco","bajar dividendo","bajar la cuota","ya tengo credito"],
      a:"Sí: podemos ayudarte a <b>refinanciar</b> o hacer <b>portabilidad financiera</b> para bajar tu tasa o tu dividendo. Cuéntanos tu caso en <a href='contacto.html'>contacto</a> o por WhatsApp."},
    primera:{keys:["primera vivienda","primera casa","comprar casa","comprar departamento","comprar mi primera"],
      a:"Para tu <b>primera vivienda</b> financiamos hasta el 90% con plazos de hasta 40 años. Lo ideal es partir simulando y luego conversamos tu caso."},
    ...
    proceso:{keys:["proceso","como funciona","pasos","etapas","como es el tramite"],
      a:"El proceso es: <b>1)</b> simulas y conversamos · <b>2)</b> reúnes documentos · <b>3)</b> evaluación y aprobación en varios bancos · <b>4)</b> tasación y escrituras · <b>5)</b> recibes las llaves."},
    ...
    gracias:{keys:["gracias","thank","genial","perfecto","listo","ok gracias"],
      a:"¡De nada! 😊 Si quieres, simula tu crédito en el <a href='simulador.html'>simulador</a> o escríbenos por WhatsApp."}
  };
  ...
  function findAnswer(text){
    ...
    return best ? best.a : "No estoy seguro de eso 🤔, pero un asesor puede ayudarte en detalle. Puedes <a href='contacto.html'>dejar tus datos</a> o escribirnos por WhatsApp. Mientras, prueba el <a href='simulador.html'>simulador</a>.";
  }
  ...
  function build(){
    ...
    // chips siempre al final del área con scroll
    function showChips(){
      const old=body.querySelector(".capi-chiprow"); if(old) old.remove();
      const row=document.createElement("div"); row.className="capi-chiprow";
      CHIPS.forEach(c=>{
        const b=document.createElement("button"); b.className="capi-chip"; b.textContent=c.t;
        b.onclick=()=>{ addMsg(c.t,"user"); reply(KB[c.id].a); };
        row.appendChild(b);
      });
      body.appendChild(row); scrollDown();
    }
    function reply(html){ setTimeout(()=>{ addMsg(html,"bot"); showChips(); },280); }
    function handleText(text){ addMsg(text,"user"); reply(findAnswer(text)); }
    ...
    function open(){
      ...
      if(!started){
        ...
        setTimeout(()=>{ addMsg("¿En qué te puedo ayudar hoy?","bot"); showChips(); },280);
      }
